...
'''
    ...
    if favorites_only and session.get('user_id'):
        sql += '''
            AND l.id IN (
                SELECT listing_id
                FROM favorites
    ...
    fav_cur = conn.cursor(dictionary=True)

    for listing in listings:
        if session.get('user_id'):
            fav_cur.execute(
                'SELECT 1 FROM favorites WHERE user_id = %s AND listing_id = %s',
                (session['user_id'], listing['id'])
            )
            listing['is_favorited'] = fav_cur.fetchone() is not None
        else:
            listing['is_favorited'] = False

    fav_cur.close()
    
    image_cur = conn.cursor(dictionary=True)

    for listing in listings:
        image_cur.execute(
            'SELECT image_path FROM listing_images WHERE listing_id = %s ORDER BY id',
            (listing['id'],)
        )
        listing['images'] = [row['image_path'] for row in image_cur.fetchall()]

    image_cur.close()

    cur.close()
    conn.close()
    return render_template('home.html', listings=listings, categories=categories, selected_category=category_id, q=query, location_query=location_query, favorites_only=favorites_only)
